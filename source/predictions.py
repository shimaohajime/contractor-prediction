...
from sklearn.metrics import adjusted_rand_score, silhouette_score, accuracy_score, f1_score, make_scorer
...
X_scaled = pd.read_csv('data/X_scaled_fill{}.csv'.format(FillType),index_col=0)
...
X_unscaled = pd.DataFrame( scaler.inverse_transform(X_scaled), columns=X_scaled.keys(), index= X_scaled.index )
...
#emb_method = TSNE(n_components=2)#PCA(n_components=2)#
    ...
    clustering = clustering_model.fit(X_scaled)#
...
for model in clustering_models:
    ...
    silhouette = silhouette_score(X_scaled, label)
...
print('------------CV--------------')
cv_result = []
cv_pred = pd.DataFrame(data = y,columns=['Outcome'], index = X_scaled.index)
for model in models:
    scores = cross_val_score(model, X_scaled, y.values.flatten(), cv=3)
    f1 = cross_val_score(model, X_scaled, y.values.flatten(), cv=3, scoring=make_scorer(f1_score))
    cv_pred[model.__class__.__name__] = cross_val_predict(model, X_scaled, y.values.flatten(), cv=3)
    cv_result.append([model.__class__.__name__, scores.mean(),f1.mean()])
    print('Model: {} CV score: {}'.format(model.__class__.__name__, scores.mean()))
cv_df = pd.DataFrame(cv_result, columns = ['model', 'accuracy', 'F1'])
...
for folds in [2,3,5,10]:
    cv_score = []
    for model in models:
        scores = cross_val_score(model, X_scaled, y.values.flatten(), cv=folds)
        cv_score.append( scores.mean())
    cv_score_all.append(cv_score)
    sample_size.append( int(len(X_scaled)/folds) )
...
cv_fold_df['training sample'] = [ int(len(X_scaled)* (folds-1)/folds) for folds in [2,3,5,10] ]
...
'''
kf = KFold(n_splits=3)
pred_proba_df = pd.DataFrame( index=X_scaled.index )
for model in models:
...
pred_proba_df['Outcome'] = y['Outcome']
...
mis_df=mis_df[['Outcome'] + [model.__class__.__name__+'_proba1' for model in models ]]
for model in models:
...
xgb_shap_df = pd.DataFrame(data=shap_values.values, columns=X_scaled.keys(), index=X_scaled.index)
...
mis_ = ["Misclassified" if i in mis_sample else "Correctly classified" for i in X_scaled.index]
...
dot_data = tree.export_graphviz(dt_model.fit(X_unscaled, y), out_file=None,feature_names=X_unscaled.keys(),class_names=True,filled=True, rounded=True,special_characters=True)
graph = graphviz.Source(dot_data)
graph.render('result/decision-tree',format='png')

dot_data_no_industry = tree.export_graphviz(dt_model.fit(X_unscaled.drop(columns=industry), y), out_file=None,feature_names=X_unscaled.drop(columns=industry).keys(),class_names=True,filled=True, rounded=True,special_characters=True)
